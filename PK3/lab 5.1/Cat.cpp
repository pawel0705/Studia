...
Cat::Cat(std::string name, int age, bool isMale, WORD color) : AnimalSprite(name, age, isMale, color)
{}

/*
Cat::Cat(const Cat &cat) : AnimalSprite(cat)
{}

Cat::Cat(Cat &&cat) : AnimalSprite(move(cat))
{}
*/
Cat::~Cat()
{}
...
{
	AnimalSprite::operator= (cat);
	return *this;
}

Cat& Cat::operator= (Cat&& cat)
{
	AnimalSprite::operator= (std::move(cat));
	return *this;
}
#pragma endregion
...
AnimalSprite::Action Cat::ActionInfo(int index)
{
...
{
	return "Cat " + AnimalSprite::DoGetInfo();
}
#pragma endregion
...
void Cat::SeeOtherAimal(const AnimalSprite& animal, float width, float height) //if see other animal run away
{
